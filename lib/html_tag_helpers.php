...
function content_tag($name, $content = null, $options = array()) {
	return "<$name" . tag_options($options) . ">" . htmlspecialchars($content) . "</$name>";
}
...
function input_tag($type, $name, $value = null, $options = array()) {
	$options = array_merge(array('type' => $type, 'name' => $name, 'id' => $name, 'value' => $value), $options);
...
function text_area_tag($name, $default = null, $options = array()) {
	...
	$options = array_merge(array('name' => $name, 'id' => $name, 'cols' => 60, 'rows' => 5), $options);
...
function radio_button_tag($name, $value, $default = false, $options = array()) {
	if ((isset($_REQUEST[$name]) and $_REQUEST[$name] == $value) or (!isset($_REQUEST[$name]) and $default)) {
...
function check_box_tag($name, $value = '1', $default = false, $options = array()) {
	if ((isset($_REQUEST[$name]) and $_REQUEST[$name] == $value) or (!isset($_REQUEST['submit']) and $default)) {
		$options = array_merge(array('checked' => 'checked'), $options);
	}
...
function label_tag($name, $text = null, $options = array()) {
	...
	$options = array_merge(array('for' => $name, 'id' => "label_for_$name"), $options);
...
function form_tag($uri = null, $options = array()) {
	...
	$options = array_merge(array('method' => 'get', 'action' => $uri), $options);
